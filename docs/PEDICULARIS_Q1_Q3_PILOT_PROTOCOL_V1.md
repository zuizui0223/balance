...
Use a two-stage qualification design rather than treating a small mechanical screen as formal evidence of selectivity.

```text
Stage A mechanical feasibility screen: ~12-20 matched plants/blocks
Stage B formal equivalence qualification: n determined by the tightest protected endpoint
```

Whenever biologically feasible, use matched flowers within plant/inflorescence. Plan Stage B from the paired-difference SD and the raw-unit equivalence margin frozen in Stage 0.

The standardized planning envelope is documented in `docs/PEDICULARIS_Q1_Q3_MINIMUM_PILOT_SIZE_V1.md` and generated by `scripts/build_pedicularis_q1_q3_equivalence_grid.py`. As a logistics envelope only, margins of 0.5, 0.4 and 0.3 paired-difference SD correspond roughly to 25, 39 and 69 complete matched pairs at one-sided alpha 0.05 and 80% power when the true leakage is zero. Raw-unit biological tolerances, not convenient standardized margins, determine the actual Stage B sample size.
