"""Canonical validation and aggregation for the BALANCE reality-pattern ledger."""
...
def load_pattern_ledger(path: Path) -> list[dict[str, str]]:
    """Load one ledger only if it satisfies the frozen one-cluster-per-row contract."""
...
def build_pattern_readout_from_rows(rows: list[dict[str, str]]) -> dict:
    """Aggregate rows already validated by :func:`load_pattern_ledger`."""
...
def build_pattern_readout(path: Path) -> dict:
    return build_pattern_readout_from_rows(load_pattern_ledger(path))
